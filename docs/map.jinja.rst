...
We will see a quick setup to configure the ``scribe`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``scribe`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``scribe`` formulas under ``/srv/salt/scribe/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/scribe/parameters/dns:domain/
    ln -s dns:domain /srv/salt/scribe/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/scribe/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/scribe/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``scribe`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``scribe`` formulas under ``/srv/salt/scribe/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/scribe/parameters/roles
...
We create a configuration for the role ``scribe/server`` in ``/srv/salt/scribe/parameters/roles/scribe/server.yaml``:
...
We create another configuration for the role ``scribe/client`` in ``/srv/salt/scribe/parameters/roles/scribe/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles scribe/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles scribe/client
...
.. code-block:: console
    ...
    mv /srv/salt/scribe/parameters/dns:domain/  '/srv/salt/scribe/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-scribe-as-json:
      test.nop:
        - name: {{ scribe | json }}
...
Here is an example based on `template-formula/scribe/config/file.sls`_:
    ...
    scribe-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='scribe-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ scribe.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``scribe`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ scribe['winner'] }}
...
.. _template-formula/scribe/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/scribe/config/file.sls
